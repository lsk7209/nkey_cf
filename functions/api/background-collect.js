...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const backgroundMode = true; // 백그라운드 모드 활성화

    console.log('백그라운드 수집 요청:', { keyword, relatedCount: related.length, autoUpdateDocuments });
    ...
    console.error('백그라운드 수집 요청 오류:', error);
    return new Response(JSON.stringify({
      success: false,
      error: String(error),
      backgroundMode: true
...
// 백그라운드 수집 작업 수행
async function performBackgroundCollection({ collectionId, keyword, related, autoUpdateDocuments, env, startTime }) {
  ...
  // OpenAPI 키 선택 함수 (save-data.js와 동일)
  function getAvailableOpenApiKey(env) {
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      if (clientId && clientSecret) {
        keys.push({ clientId, clientSecret });
      }
    }
